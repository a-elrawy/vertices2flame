...
class MooflDataset(Dataset):
    def __init__(self,input_folder,in_memory=False,mode="train",read_audio=True, 
                                    file_name=False, return_exp=False, return_face=False):

        self.input_folder = input_folder
        self.model = EncodecModel.encodec_model_24khz()

        self.return_exp = return_exp
        self.return_face = return_face
        ...
        self.face_files = [os.path.join('outfaces', f"{os.path.splitext(os.path.basename(f))[0]}_white.mp4") for f in self.audio_files]
        ...
        self.face_files = [f for i, f in enumerate(self.face_files) if np.isclose(self.video_fps[i], fps, atol=1)]
        ...
        if self.in_memory:
            ...
            if self.return_face:
                self.face_files = [read_video(f) for f in self.face_files]
    ...
    def __getitem__(self,idx):
        ...
        if not self.in_memory:
            ...
            face = read_video(self.face_files[idx]) if self.return_face else None
        else:
            exp = self.exp_files[idx]
            pose = self.pose_files[idx]
            audio_codes = self.audio_files[idx] if self.read_audio else None
            face = self.face_files[idx] if self.return_face else None
        ...
        if self.return_face:
            return face, face[3], self.audio_files[idx]
...
def get_dataloader_vox(config):
    if config.arch == "flame":
        dataset = MooflDataset("./VoxCeleb",
                                mode="train", read_audio=config.read_audio, file_name=config.visualize, in_memory=True, return_exp=True)
    elif config.arch == "face" or config.arch == "stage1_face":
        dataset = MooflDataset("./VoxCeleb",
                                mode="train", read_audio=config.read_audio, file_name=config.visualize, in_memory=False, return_face=True)
    else:
        dataset = MooflDataset("./VoxCeleb",
                            mode="train", read_audio=config.read_audio, file_name=config.visualize, in_memory=True)

    collate_fn = None
    if config.arch in ['stage1_vocaset', 'stage1_face']:
        collate_fn = collate_fn_stage1 if config.batch_size > 1 else None
    elif config.arch in ['stage2']:
